    ...
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit = {handleSubmit} className='flex flex-col gap-4'>
        ...
        <button disabled= {loading} className='bg-slate-700 text-white rounded-lg p-3 uppercase hover:opacity-80 disabled:opacity-50'>
          ...
          <Link className= 'bg-green-700 text-white p-3 rounded-lg uppercase text-center hover:opacity-50' to= {"/create-listing"}>Create Listing</Link>
      </form>
      ...
      <p className='text-red-700 font-semibold mt-5'>{showListingsError ? "Error showing listings" : ""}</p>
      {userListing && userListing.length > 0 && 
         
        <div className='flex flex-col gap-4'><h1 className='text-center mt-7 text-2xl font-semibold'>Your Listings</h1>{userListing.map((listing) => (
        <div key= {listing._id} className='flex justify-between items-center border rounded-lg p-3 gap-4'>
          <Link to={`/listing/${listing._id}` }> <img src= {listing.imageURLs[0]} alt="listing" className='h-16 w-16 object-contain' />
          </Link>
          <Link className='text-slate-700 font-semibold flex-1 truncate hover:underline' to={`/listing/${listing._id}`}> <h1>{listing.name}</h1>
          </Link>
        <div className='flex flex-col'>
          <button onClick={() => handleDeleteListing(listing._id)} className='text-red-700 uppercase'>Delete</button>
          <Link to={`/edit-listing/${listing._id}`}><button className='text-green-700 uppercase'>Edit</button>
          </Link>
